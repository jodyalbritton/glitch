...
const baseUrl = config.get("stApi.apiUrl");
...
function signatureIsVerified (req) {
  // For dev purposes only; real apps should always verify the http
  // signature of inbound requests.
  const publicKey =process.env.SECRET;
  if (config.get("stApi.verifyRequests") === false) {
    return true;
  }
...
/**
 * Creates a recurring (every 10 minutes) scheduled execution for this
 * installed app.
 *
...
function createSchedule (installedApp, token) {
  ...
  scheduling.deleteSchedules(installedApp.installedAppId, baseUrl, token)
    .then(function (resp) {
